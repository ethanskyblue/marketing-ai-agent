...
app.post('/api/pdf', (req, res) => {
  ...
  try {
    ...
    messages.forEach((m, idx) => {
      ...
      // Clean text: strip HTML tags and markdown
      const clean = (m.text || '')
        .replace(/▶\s?/g, '> ')
        .replace(/<[^>]+>/g, '')
        .replace(/\*\*(.*?)\*\*/g, '$1')
      ...
      // Measure text height
      const textH = doc.font('Regular').fontSize(10)
                       .heightOfString(clean, { width: CW - 20 });
      const blockH = textH + 36; // label + padding

      // Page break
      if (y + blockH > doc.page.height - 40) {
        doc.addPage({ margin: 0 });
        y = 24;
      }

      // Background
      doc.roundedRect(ML, y, CW, blockH, 6).fill(bgColor);
      ...
      doc.font('Regular').fontSize(10).fillColor('#2a2a3e')
         .text(clean, ML + 10, y + 24, { width: CW - 20, lineGap: 2 });
