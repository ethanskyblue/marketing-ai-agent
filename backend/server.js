...
// ─── Trust Proxy (Cloudtype 프록시 환경 필수) ────────────────────────────────
...
// ─── Rate Limit (express-rate-limit 없이 직접 구현) ──────────────────────────
...
function loadCSVData() {
  try {
    ...
    const genderDist = {};
    customerData.forEach(r => { genderDist[r.Gender] = (genderDist[r.Gender] || 0) + 1; });

    const countryDist = {};
    customerData.forEach(r => { countryDist[r.Country] = (countryDist[r.Country] || 0) + 1; });
    ...
    const quarterDist = {};
    customerData.forEach(r => { quarterDist[r.Signup_Quarter] = (quarterDist[r.Signup_Quarter] || 0) + 1; });

    dataStats = {
      total, churned, churnRate: ((churned / total) * 100).toFixed(1),
      avgAge, avgLTV, avgOrders, avgCartAbandonment, avgDiscount, avgEmailOpen,
      genderDist, topCountries, churnByAge, highValueCount,
      topLTVThreshold: topLTVThreshold.toFixed(0),
      avgPaymentDiv, avgMobileUsage, quarterDist,
...
app.post('/api/chat', apiLimiter, async (req, res) => {
  try {
    ...
    const apiKey = rawKey.split('').filter(c => c.charCodeAt(0) > 31 && c.charCodeAt(0) < 127).join('').trim();
    if (!apiKey) {
...
【응답 지침】
- 한국어와 영어를 모두 사용하여 답변하세요 (한국어 주, 영어 부)
...
- 답변 끝에 후속 질문을 제안하세요`;
    ...
    // node-fetch로 직접 Anthropic API 호출 (SDK 미사용)
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return res.status(500).json({ error: 'API 키가 유효하지 않습니다. Cloudtype 환경변수를 확인해주세요.' });
      }
...
app.post('/api/export-pdf', pdfLimiter, async (req, res) => {
  try {
    ...
    const useFont = (size = 12) => {
      fontAvailable ? doc.font('Korean').fontSize(size) : doc.font('Helvetica').fontSize(size);
    };
    ...
    chatHistory.forEach((item) => {
      ...
      useFont(10);
      doc.fillColor('#333333').text(textContent, boxX + 8, doc.y + 20, { width: boxW - 16, lineGap: 2 });
      doc.y = doc.y + boxH + 8;
    });
    ...
    useFont(8);
    doc.fillColor('#999999').text('Generated by Marketing AI Agent | Powered by Claude AI', 50, doc.page.height - 40, { align: 'center' });
    doc.end();
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', dataLoaded: customerData.length > 0, totalRecords: customerData.length });
});
